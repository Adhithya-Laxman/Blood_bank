...
class Blood(db.Model):
    ...
    transfusions_donor = db.relationship('TransfusionDonor', backref= 'blood')
...
class Donor(db.Model):
    ...
    mobile = db.Column(db.String(13), nullable=False)
    transfusions_donor = db.relationship('TransfusionDonor', backref= 'donor')
...
class Hospital(db.Model):
    ...
    transfusions_donor = db.relationship('TransfusionDonor', backref= 'hospital')
...
class TransfusionDonor(db.Model):
    __tablename__ = 'transfusion_donor'
    __table_args__ = {'extend_existing': True}

    did = db.Column(db.Integer, db.ForeignKey('donor.did'), primary_key=True)
    bid = db.Column(db.Integer, db.ForeignKey('blood.bid'), primary_key=True)
    hid = db.Column(db.Integer, db.ForeignKey('hospital.hid'), primary_key=True)
    dot = db.Column(db.Date, nullable=False)
